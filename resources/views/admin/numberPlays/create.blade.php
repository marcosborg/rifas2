...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.numberPlay.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.number-plays.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('number') ? 'is-invalid' : '' }}" name="number_id" id="number_id" required>
                    @foreach($numbers as $id => $entry)
                        <option value="{{ $id }}" {{ old('number_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
