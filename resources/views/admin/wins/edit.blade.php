...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.win.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.wins.update", [$win->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('star') ? 'is-invalid' : '' }}" name="star_id" id="star_id" required>
                    @foreach($stars as $id => $entry)
                        <option value="{{ $id }}" {{ (old('star_id') ? old('star_id') : $win->star->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('star'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('star_play') ? 'is-invalid' : '' }}" name="star_play_id" id="star_play_id" required>
                    @foreach($star_plays as $id => $entry)
                        <option value="{{ $id }}" {{ (old('star_play_id') ? old('star_play_id') : $win->star_play->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $win->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
